...
public class ReceiveAndDumper extends Thread {
     ...
     * @param sType type of data coming over TCP client's socket.
    ...
    public ReceiveAndDumper(int sampaPort,
        ...
        if (sType.isDSP()) {
            sDecoder = new DspDecoder(verbose);
        }
        else {
            sDecoder = new DasDecoder(verbose, streamId, byteSize);
        }
    }
    ...
    public void run() {
        ...
        try {
            ...
            SRingRawEvent rawEvent = new SRingRawEvent(SType.DAS);
            ...
            do {
                ...
                } while ( !(sDecoder.isFull() || ((streamFrameLimit != 0) && (frameCount >= streamFrameLimit))) );

                if (sType.isDSP()) {
                    if (rawEvent.isFull()) {
                        // Update the block number since the event becomes full once
                        // a complete block of data has been written into it.
                        rawEvent.setBlockNumber(sDecoder.incrementBlockCount());
                    }
                }
                else {
                    ((DasDecoder) sDecoder).transferData(rawEvent);
